...
###############################################
get_time_ms() {
    # Try GNU date with %3N (milliseconds) - check if it produces valid numeric output
    local test_output
    test_output=$(date +%s%3N 2>/dev/null || echo "fail")
    
    if [[ "$test_output" =~ ^[0-9]+$ ]]; then
        # GNU date with millisecond support available
        echo "$test_output"
    else
